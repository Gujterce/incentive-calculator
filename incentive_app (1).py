...
def hyterce_calculator():
    ...
    product = st.selectbox("Select Product", ["", "Syrup", "Drops"], index=0)
    total_units = st.number_input(
        "Total primary units (Jun–Aug 2025)", min_value=0, step=1, value=0
    )
    months = st.number_input(
        "Number of months (default 3)", min_value=1, max_value=3, value=3
    )
    if product:
...
def mr_annual_incentive():
    ...
    pcpm = st.number_input("PCPM (Lakhs)", min_value=0.0, step=0.01, value=0.0)
    achievement = st.number_input("Achievement %", min_value=0.0, step=0.1, value=0.0)
    salary = st.number_input(
        "Monthly gross salary (Rs)", min_value=0.0, step=1000.0, value=0.0
    )
    group = ""
    if pcpm:
        if pcpm < 1.5:
...
def mr_volume_incentive():
    ...
    period = st.selectbox("Period", ["", "Quarter", "Annual"], index=0)
    pcpm = st.number_input("PCPM (Lakhs)", min_value=0.0, step=0.01, value=0.0)
    achievement = st.number_input("Achievement %", min_value=0.0, step=0.1, value=0.0)
    sale = st.number_input("Net primary sale (Rs)", min_value=0.0, step=1000.0, value=0.0)
    group = ""
    if pcpm:
        if pcpm < 1.5:
            group = "Group A"
...
def mr_brand_incentive():
    ...
    pcpm = st.number_input("Annual PCPM (Lakhs)", min_value=0.0, step=0.01, value=0.0)
    num_groups = st.number_input(
        "Number of brand groups achieving 100% target", min_value=1, max_value=11, step=1, value=1
    )
    group = ""
    if pcpm:
        if pcpm < 1.5:
...
def mr_quarterly_brand_incentive():
    ...
    pcpm = st.number_input("PCPM for the quarter (Lakhs)", min_value=0.0, step=0.01, value=0.0)
    num_brands = st.number_input(
        "Number of brands achieving 100% target", min_value=1, max_value=4, step=1, value=1
    )
    group = ""
    if pcpm:
        if pcpm < 1.5:
...
def manager_incentive(role_name, threshold, high_multiplier):
    ...
    total_mr_incentive = st.number_input(
        "Total MR incentive amount (Rs)", min_value=0.0, step=1000.0, value=0.0
    )
    num_mrs = st.number_input(
        "Number of MRs in team", min_value=1, step=1, value=1
    )
    pct_mrs = st.number_input(
        "Percentage of MRs earning incentives (%)", min_value=0.0, max_value=100.0, step=1.0, value=0.0
    )
...
def resplash_incentive():
    ...
    base_units = st.number_input(
        "Base sale units (Mar–Jun 2024)", min_value=0, step=1, value=0
    )
    current_units = st.number_input(
        "Sale units (Mar–Jun 2025)", min_value=0, step=1, value=0
    )
